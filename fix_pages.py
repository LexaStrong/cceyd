...
# We need to replace the content dictionary values in build_site.py
# Using regex to find the dictionary values is tricky. Let's find the assignment of `pages['index.html'] = {` and extract the content section.
# A more robust way is to just use string replace on the known structure or write a quick regex.

content = re.sub(r'(\'index\.html\': \{\n\s*\'title\': \'CCEYD \\| Home\',\n\s*\'desc\': \'Home page of CCEYD\',\n\s*\'content\': """)[\s\S]*?(""")(?=\n\s*\})', rf'\1\n{index_html}\n\2', content)

content = re.sub(r'(\'about\.html\': \{\n\s*\'title\': \'About Us \\| CCEYD\',\n\s*\'desc\': \'About CCEYD\',\n\s*\'content\': """)[\s\S]*?(""")(?=\n\s*\})', rf'\1\n{about_html}\n\2', content)
